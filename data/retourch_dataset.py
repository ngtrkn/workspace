...
def variable_mask_collate_fn(batch):
    ...
    max_n_masks = max([sum([item.shape[0] for item in batch[i][1]]) for i in range(batch_size)])
    ...
    for i, (image, label) in enumerate(batch):
        ...
        for j in range(max_n_masks):
            masks_list[j][i, :, :] = batch[i][1][0][j]

    return images, masks_list


class RetourchDataset(BaseDataset):
    def __init__(
        self,
        *args,
        in_channels=1,
        **kwargs,
        ...
        super(RetourchDataset, self).__init__(*args, **kwargs)
